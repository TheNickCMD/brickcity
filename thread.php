...
$owner = $bop->get_user($thread->author, ["username", "id", "posts", "joined", "upvotes", "admin"]);
$localUser = $bop->local_info();
if(!$bop->loggedIn()) {
if($bop->isBanned(0) && !$bop->isAllowed(0))
{
require("/var/www/html/error/404.php");
die();
}
...
<meta property="og:image" content="https://storage.vertineer.com/avatars/<?=$avatar->cache?>.png">
...
<div class="content">
  <div class="col-1-1">
		<div style="background-color:#f7f7f7;height:22.5px;border: 1px solid #000000; width:calc(100% - 12px); padding:5px; margin-bottom: 10px;border-radius:5px;">
		...
		<div class="forum-topic">
			<div class="title">
				...
				if($bop->logged_in())
				{
					...
					{
						?>
						<i class="fa fa-thumbs-up upvote hover" aria-hidden="true" title="Upvote this post" req="0"></i>
						<?php
				...
				<div class="col-3-12">
					<div class="centered">
						<a href="/profile/<?=$owner->id?>">
							<img src="https://storage.vertineer.com/avatars/<?=$avatar->cache?>.png" style="margin-top:10px;width:100%;" />
							<br><br>
...
<div style="
    ...
    margin-top:  2px;
    margin-bottom: 2px;
"><i class="fas fa-gavel"></i> Administrator</div>
								<?php } ?>
					</div>
				</div>
				<div class="col-9-12">
				...
				if($bop->logged_in()) {
					if(intval($localUser->admin) > 0)
					{
						...
						<div class="col-1-1">
							...
							<button class="button danger" id="lock"><?=($thread->locked == "0") ? "Lock" : "Unlock"?></button>
							<button class="button danger" id="pin"><?=($thread->pinned == "0") ? "Pin" : "Unpin"?></button>
						</div>
						<script>
						$(document).ready(function(){
							...
							$("#lock").click(function(){
								$.post("/admin/lock_thread.php", {id: <?=$thread->id?>}, function(reply){
									location.reload();
								});
							});
							$("#pin").click(function(){
								$.post("/admin/pin_thread.php", {id: <?=$thread->id?>}, function(reply){
									location.reload();
								});
							});
						});
						...
						<div class="col-3-12">
							<div class="centered">
								<a href="/profile/<?=$user->id?>">
									<img src="https://storage.vertineer.com/avatars/<?=$user_avatar->cache?>.png" style="margin-top:10px;width:100%;" />
									<br><br>
		...
		if($bop->logged_in() && (($thread->deleted == "0" && intval($localUser->admin) == 0) || intval($localUser->admin) >= 1)){
			...
			<div class="col-1-1">
				<div class="col-4-12">
					...
					{
						?>
						<a href="/forum/thread.php?id=<?=$thread->id?>&p=<?=$page - 1?>" class="button success">Previous Page</a>
						<?php
					}
					?>
				</div>
				...
				<div class="col-4-12">
					...
					{
						?>
						<a href="/forum/thread.php?id=<?=$thread->id?>&p=<?=$page + 1?>" class="button success" style="float:right;">Next Page</a>
						<?php
					}
					?>
				</div>
