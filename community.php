  ...
  <div class="page-title"><?=htmlentities($community->name)?> [<font color="#8973f9"><?=htmlentities($community->shorthand)?></font>]
    ...
    if($bop->logged_in()) {
      ...
      if($founder->id == $localInfo->id) {
        ?>
        <div class="hover-edit" style="float:right;padding-right:10px;font-size:1.5rem;"><a href="/community/settings.php?id=<?=$community->id?>" title="Edit this community."><i class="far fa-edit"></i></a></div></div>
        <?php
      }
    }
    ?>
...
<div class="col-8-12">
  <div class="card b centered">
    ...
    <div class="body" style="height:450px;text-align:left;">
      <?php if($community->desc_hidden == 0) { ?>
      <?=nl2br($bop->bbCode(htmlentities($community->desc)))?>
      <?php } else { ?>
      [ Deleted ]
      <?php } ?>
      <?php if($bop->logged_in() && $localUser->admin >= 1) { ?>
      <br>
      <a class="button danger" href="/admin/hideDesc.php?id=<?=$community->id?>">Delete</a>
      <?php } ?>
    </div>
  </div>
</div>
...
<div class="col-1-1">
  <div class="card b">
    <div class="top centered">Store</div>
    <div class="body">
      <div class="col-1-1">
	  &nbsp;
	  <?php if($owner->premium >= 1) { ?>			
          <div class="shop-item-container ">				<div class="shop-item">					<div class="preview">						<img src="https://storage.vertineer.com/thumbnails/320.png">											</div>					<div class="info">						<div class="name">Black Vertineer Hoodie</div>						<div class="buy"><div class="price ">V 5</div><a href="/item/320" class="button ">View</a></div>						<a href="/profile/1" class="creator">Vertineer</a>					</div>				</div>			</div>
      <?php } ?>
	  
<?php
$owner = $bop->get_user($community->id, ["premium"]);
