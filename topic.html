  ...
  <div class="col-1-1">
    ...
    if($count == 0)
    {
      ...
      <?php
            ...
            {
              $thread = (object) $a;
              if($bop->user_exists($thread->author)) {
              ...
              <div class="card border <?=($thread->pinned == 1) ? "pinned" : ""?> <?=($thread->locked == 1) ? "locked" : ""?>" style="margin-bottom:7px;">
				<div class="col-8-12">
          <img src="https://storage.vertineer.com/heads/<?=$avatar->headshot?>.png" height="50" style="float:left;margin-left:2.5px;margin-right:10px;border-radius:100px;">
					<div class="subforum-title">
						<?php
            if($thread->pinned == 1)
            {
              ?>
              <span style="color:black"><i class="fas fa-thumbtack" aria-hidden="true"></i></span>
              <?php
            }
            if($thread->locked == 1)
            {
              ?>
              <span style="color:black"><i class="fas fa-lock" aria-hidden="true"></i></span>
              <?php
            }
            ?><a href="/forum/thread.php?id=<?=$thread->id?>"><?=($thread->deleted == 0) ? htmlentities($thread->title) : "[Deleted]"?></a>
					</div>
        ...
        <div class="centered col-2-12">
          ...
          if($count == $limit) {
            ?>
            <span style="float:right;"><a href="/forum/topic.php?id=<?=$topic->id?>&p=<?=$page + 1?>" class="button success">Next Page</a></span>
            <?php
          }
          ?>
		  </div>
