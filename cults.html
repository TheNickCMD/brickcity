...
if($bop->logged_in())
{
  ...
  <div class="content">
    ...
    <?php
    $communities = $bop->query("SELECT DISTINCT(community.id), community.desc, community.pending, community.name, community.shorthand, community.founder, community.created, community.cache, community.verified FROM community INNER JOIN community_member ON community.id = community_member.cid WHERE community_member.uid=?", [$localInfo->id], true);
...
<?php
$popular = $bop->query("SELECT * FROM community ORDER BY members DESC LIMIT 0, 4", [], true);
...
?>
<a class="button success" href="/community/groups.php">View All</a>
<?php
$bop->footer();
?>
